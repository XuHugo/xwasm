...
lazy_static!{
    pub static ref STATE_MEM: Arc<Mutex<State>> = Arc::new(Mutex::new(State{db: DB::open("statedb", rusty_leveldb::in_memory()).unwrap()}));
}

lazy_static!{
    
    pub static ref STATE_DB:  Arc<Mutex<State>> = Arc::new(Mutex::new(State{db: DB::open(STATE_PATH, State::opts()).unwrap()}));
}
...
impl State{
    ...
    fn compact(&mut self, from: &str, to: &str) {
        self.db.compact_range(from.as_bytes(), to.as_bytes()).unwrap();
    }
}
