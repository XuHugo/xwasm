...
impl Metadata{
    ...
    pub fn call(&self, address:Address, amount:u64, func:String, arg:String)->Result<(), WasmError>{
        Ok(())
    }
...
impl VM for WasmtimeRuntime {
    fn execute(func_name: &str, context: Context, binary: &[u8], amount: u64) -> anyhow::Result<WasmResult> {
        let ret = wasmtime_vm::execute(func_name, context, binary, amount);
        ret
    }
}