...
pub struct Scheduler {
    ...
    pub txn_dependency: Vec<CachePadded<Mutex<Vec<TxnIndex>>>>,
...
impl Scheduler {
    ...
    pub fn next_task(&self) -> SchedulerTask {
        //print!("NT({:?}):",std::thread::current().name()); //tracestm
        loop {
            if self.done() {
                //println!("NT({:?})-done",std::thread::current().name()); //tracestm
                // No more tasks.
            ...
            if idx_to_validate < idx_to_execute {
                if let Some((version_to_validate, guard)) = self.try_validate_next_version() {
                    //println!("NT({:?})-vt{:?}",std::thread::current().name(),version_to_validate);//tracestm
            ...
            {
                //println!("NT({:?})-et{:?}-{:?}",std::thread::current().name(),version_to_execute,maybe_condvar);//tracestm
